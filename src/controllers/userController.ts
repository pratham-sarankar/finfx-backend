...
export const createUser = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!fullName || !email || !phoneNumber || !profilePicture || !password) {
    ...
    const user = await User.create({ fullName, email, phoneNumber, profilePicture, password });
    const userObj = user.toObject();
    const { _id, __v, password: pwd, ...rest } = userObj;
    const transformedUser = { ...rest, id: _id };
    res.status(201).json({ success: true, message: "User created successfully", data: transformedUser });
  } catch (error) {
    next(error);
  }
};
...
export const updateUser = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const { fullName, email, phoneNumber, profilePicture, password } = req.body;
    if (!fullName || !email || !phoneNumber || !profilePicture) {
      throw new AppError("All fields except password are required", 400, "missing-fields");
    }
    const updateData: any = { fullName, email, phoneNumber, profilePicture };
    if (password) {
      updateData.password = await bcrypt.hash(password, 10);
    }
    const user = await User.findByIdAndUpdate(id, updateData, { new: true, runValidators: true, context: 'query' }).select("-__v -password");
    if (!user) throw new AppError("User not found", 404, "user-not-found");
    const userObj = user.toObject();
    const { _id, __v, password: pwd, ...rest } = userObj;
    const transformedUser = { ...rest, id: _id };
    res.status(200).json({ success: true, message: "User updated successfully", data: transformedUser });
  } catch (error) {
    next(error);
  }
};
